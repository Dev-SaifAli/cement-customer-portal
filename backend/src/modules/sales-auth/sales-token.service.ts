...
import type { SalesAuthTokenPayload } from './sales-auth.types.js';

const algorithm = 'HS256';

interface JwtPayload extends SalesAuthTokenPayload {
  iat: number;
  exp: number;
}

export class SalesTokenService {
  createToken(payload: SalesAuthTokenPayload) {
  ...
  verifyToken(token: string): SalesAuthTokenPayload {
    const secret = this.getSecret();
    ...
    const now = Math.floor(Date.now() / 1000);
    if (payload.exp <= now || payload.type !== 'sales' || !payload.sub) {
      throw new AppError('Sales authentication is required.', 401, 'SALES_AUTH_REQUIRED');
    }
  ...
  private getSecret() {
    if (!env.JWT_SECRET) {
      throw new AppError(
        'Sales authentication is not configured.',
        500,
        'SALES_AUTH_NOT_CONFIGURED',
      );
    }
...
export const salesTokenService = new SalesTokenService();
