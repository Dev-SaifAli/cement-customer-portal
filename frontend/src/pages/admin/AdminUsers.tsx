    ...
    <main className="mx-auto w-full max-w-[1500px] space-y-5 p-4 lg:p-7">
      ...
      <section className="customer-card overflow-hidden rounded-xl border shadow-sm">
        <div className="customer-border-soft flex flex-wrap items-center gap-3 border-b p-4">
          ...
          <NativeTomSelect className="customer-input h-10 rounded-lg border px-3 text-sm" value={role} onChange={(event)=>{setRole(event.target.value as InternalUserRole|'');setPage(1)}}><option value="">All roles</option>{roles.map((item)=><option key={item.value} value={item.value}>{item.label}</option>)}</NativeTomSelect>
          <NativeTomSelect className="customer-input h-10 rounded-lg border px-3 text-sm" value={status} onChange={(event)=>{setStatus(event.target.value as InternalUserStatus|'');setPage(1)}}><option value="">All statuses</option><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></NativeTomSelect>
      ...
      <Modal open={formOpen} title={editing ? 'Edit User' : 'Add User'} onClose={()=>setFormOpen(false)} footer={<><Button variant="secondary" onClick={()=>setFormOpen(false)}>Cancel</Button><Button loading={saving} onClick={()=>void save()}>{editing?'Save Changes':'Create User'}</Button></>}>
        <form className="grid gap-4 sm:grid-cols-2" onSubmit={submit}>
          ...
          <FormField label="Role *"><NativeTomSelect className="customer-input h-11 w-full rounded-lg border px-3" value={form.role} onChange={(event)=>setForm({...form,role:event.target.value as InternalUserRole})}>{roles.map((item)=><option key={item.value} value={item.value}>{item.label}</option>)}</NativeTomSelect></FormField>
          <FormField label="Status"><NativeTomSelect className="customer-input h-11 w-full rounded-lg border px-3 disabled:cursor-not-allowed disabled:opacity-60" disabled={editing?.id===currentUser?.id} title={editing?.id===currentUser?.id?'You cannot deactivate your own account.':undefined} value={form.status} onChange={(event)=>setForm({...form,status:event.target.value as InternalUserStatus})}><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></NativeTomSelect>{editing?.id===currentUser?.id && <span className="customer-muted block text-xs">Your own account must remain active.</span>}</FormField>
