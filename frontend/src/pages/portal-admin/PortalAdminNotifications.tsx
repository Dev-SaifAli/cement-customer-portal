    ...
    <main className="mx-auto w-full max-w-[1500px] space-y-5 p-4 lg:p-7">
      ...
      <section className="customer-card overflow-hidden rounded-xl border shadow-sm">
        <div className="customer-border-soft flex flex-wrap items-center gap-3 border-b p-4">
          <select className="customer-input h-10 rounded-lg border px-3 text-sm" value={audience} onChange={(event)=>{setAudience(event.target.value as GlobalNotificationAudience|'');setPage(1)}}><option value="">All audiences</option><option value="CUSTOMER">Customers</option><option value="SALES">Internal users</option></select>
          <select className="customer-input h-10 rounded-lg border px-3 text-sm" value={status} onChange={(event)=>{setStatus(event.target.value as GlobalNotificationStatus|'');setPage(1)}}><option value="">All statuses</option><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></select>
      ...
      <Modal open={formOpen} title={editing ? 'Edit Global Notification' : 'New Global Notification'} onClose={()=>setFormOpen(false)} footer={<><Button variant="secondary" onClick={()=>setFormOpen(false)}>Cancel</Button><Button loading={saving} onClick={()=>void save()}>{editing?'Save Changes':'Create Notification'}</Button></>}>
        <form className="space-y-4" onSubmit={submit}>
          ...
          <div className="grid gap-4 sm:grid-cols-2"><label className="space-y-1.5"><span className="customer-text text-sm font-semibold">Audience *</span><select className="customer-input h-11 w-full rounded-lg border px-3" value={form.audience} onChange={(event)=>setForm({...form,audience:event.target.value as GlobalNotificationAudience,targetRoles:[]})}><option value="CUSTOMER">Customers</option><option value="SALES">Internal users</option></select></label><label className="space-y-1.5"><span className="customer-text text-sm font-semibold">Status</span><select className="customer-input h-11 w-full rounded-lg border px-3" value={form.status} onChange={(event)=>setForm({...form,status:event.target.value as GlobalNotificationStatus})}><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option></select></label></div>
